...
ローカルの Ollama を使い、外国語の記事に日本語の **翻訳** を付け、長い本文を短い **要約** にまとめます。翻訳が有効なフィードでは記事の **タイトル** を日本語にし、本文が短い記事はその全文も翻訳します。本文が長い記事は全文翻訳ではなく、日本語の **要約** に置き換えます。母語のフィードでは、長い記事を要約するだけで翻訳はしません。登録時にフィードの言語を判定し、ネイティブ言語（`native_lang`）と異なる、あるいは判定できないフィードは **翻訳を自動で有効** にします。

> [!IMPORTANT]
> 翻訳・要約はいずれもフィードの **要約（summarize）** が有効なときにだけ生成されます。要約は既定では無効なので、翻訳・要約を使いたいフィードは Web UI（`/feeds`）で要約を ON にしてください（翻訳の有効/無効も同じ画面でフィード単位に切り替えられます）。
